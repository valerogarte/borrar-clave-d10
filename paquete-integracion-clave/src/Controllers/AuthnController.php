...
class AuthnController
{
    ...
    public function login($request): void
    {
        ...
        $this->auth->requireAuth([
            'saml:Extensions' => $this->generateAttributes($request),
            'saml:AuthnContextClassRef' => $this->getEidasLoA($request),
            'saml:AuthnContextComparison' => 'minimum',
            'ForceAuthn' => $this->getForceAuthn($request),
            'ReturnTo' => $this->assertionUrl . '/profile',
    ...
    private function generateAttributes($request): array
    {
        ...
        foreach ($idpAttrs as $idpAttr) {
            if (isset($request[$idpAttr]) && $request[$idpAttr] === 'off') {
                $ce1 = $dom->createElementNS($ns_eidas, 'eidas:RequestedAttribute');
                $ce1->setAttribute('FriendlyName', $idpAttrs[$idpAttr . '.name']);
                $ce1->setAttribute('Name', $idpAttrs[$idpAttr . '.uri']);
                $ce1->setAttribute('NameFormat', $idpAttrs[$idpAttr . '.nameFormat']);
                $ce1->setAttribute('isRequired', 'false');
                $attributes_array[] = $ce1;
            }
        }
    ...
    {
        return $request['loa'] ?? 'http://eidas.europa.eu/LoA/low'; // NOSONAR excluir norma de https
    }
     ...
     * @return bool
     */
    private function getForceAuthn(array $request): bool
    {
        return isset($request['forceauthn']) && $request['forceauthn'] === 'true';
    }
